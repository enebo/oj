package oj.rails;

import oj.OjLibrary;
import org.jruby.anno.JRubyMethod;
import org.jruby.runtime.ThreadContext;
import org.jruby.runtime.builtin.IRubyObject;

public class RubyOjRailsEncoderReplacements {

    @JRubyMethod(name="use_standard_json_time_format=")
    public IRubyObject set_use_standard_json_time_format(ThreadContext context, IRubyObject state) {
        state = context.runtime.newBoolean(state.isTrue());
        // FIXME:
        //self.getInstanceVariables().setInstanceVariable("@use_standard_json_time_format", state);

        return state;
    }

    @JRubyMethod(name="escape_html_entities_in_json=")
    public IRubyObject set_escape_html_entities_in_json(ThreadContext context, IRubyObject state) {
        state = context.runtime.newBoolean(state.isTrue());
        // FIXME:
        //self.getInstanceVariables().setInstanceVariable("@use_standard_json_time_format", state);

        return state;
    }
    @JRubyMethod(name="time_precision=")
    public IRubyObject set_time_precision(ThreadContext context, IRubyObject prec) {
        // FIXME:
        //self.getInstanceVariables().setInstanceVariable("@use_standard_json_time_format", prec);
        OjLibrary.default_options.sec_prec = prec.convertToInteger().getIntValue();

        return prec;
    }
}
